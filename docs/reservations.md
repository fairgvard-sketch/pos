...
шаге «точное время» выбирает, где сесть: чип «ללא העדפה» + чип на каждую зону.
Одна зона (066 создаёт всем «Зал») — выбора нет, поведение прежнее.
...
- В **instant-режиме** `_pick_tables` подбирает стол только в выбранной зоне;
  доступность слотов на гостевой странице пересчитывается по зоне
  (`GET ?loc&date&party&zone=<uuid>`). Зона занята целиком → `full_slot`.

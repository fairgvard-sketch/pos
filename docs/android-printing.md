...
это команда по тому же каналу, что печать. Порядок путей
(`src/lib/cashDrawer.ts`):

1. `KassaAndroid.openDrawer(jobId)` — мост v4 (APK 1.7+): штатный
   `openDrawer` сервиса Sunmi через рефлексию, при его отсутствии —
   импульс изнутри APK;
2. импульс `ESC p m t1 t2` (`1B 70 00 19 FA`) через `printBase64` — работает
   на ЛЮБОМ уже установленном APK, поэтому обновление ради ящика не
   обязательно. В задании нет ни растра, ни отреза: бумага не двигается;
3. `rawbt:base64,…` с тем же импульсом;
4. пути нет (браузер без RawBT) — касса честно говорит об этом и в
   настройках, и тостом.

Контакт разъёма (2 по умолчанию, 5 у части боксов) выбирается в
Настройках → Устройство → «Денежный ящик»; нестандартный контакт всегда
идёт импульсом, так как нативный вызов номер контакта не принимает.

Ограничение честности: подтвердить ФИЗИЧЕСКОЕ открытие касса не может —
датчика на этом пути нет, а колбэк принтера сообщает лишь о принятии
команды. Поэтому для ящика `awaitPrintResult` работает в режиме
«молчание = успех» (таймаут 3 с) и ошибка показывается только при явном
отказе моста. Реальную проверку даёт ручной прогон на T2.

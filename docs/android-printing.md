...
`src/lib/orientation.ts`: он подписан на `deviceStore` и вызывает применение
на старте кассы, при смене настройки и после восстановления серверного
snapshot — отдельного сохранения в APK нет, источник истины — web.

Пути применения:

- мост v3+: `KassaAndroid.setOrientation('auto'|'landscape'|'portrait')` →
  `requestedOrientation` (SENSOR_* — оба разворота выбранной оси); работает
  только для доверенного origin;
- браузер/PWA: Screen Orientation API как best-effort — `lock()` обычно
  отвергается без fullscreen;
- hint настройки честно сообщает доступность: мост v3 / «по возможности» в
  браузере / «обновите приложение кассы», если мост старый или API нет.
...
## Денежный ящик (144, мост v4)

Денежный ящик подключён к порту принтера, поэтому «открыть ящик» —
это команда по тому же каналу, что печать.

**Полевой факт (T2 mini, APK 1.3, 06.08.2026): импульс ESC/POS, отправленный
обычным заданием печати, сервис Sunmi ПРИНИМАЕТ, но ящик не щёлкает.** Сырая
команда `ESC p` через transaction-буфер до порта не доходит. Поэтому мост v4
(APK 1.7+) обязателен для работы ящика на Sunmi, а не «предпочтителен».

Порядок путей (`src/lib/cashDrawer.ts`):

1. `KassaAndroid.openDrawer(jobId, pin)` — мост v4: штатный `openDrawer`
   сервиса Sunmi (рефлексия — метода нет в части прошивок), фолбэк —
   импульс НАПРЯМУЮ через `sendRAWData` в обход буфера печати. Контакт 5
   идёт сразу импульсом: нативный вызов бьёт только в штатный контакт;
2. импульс через `printBase64` — единственный путь на APK < 1.7. На T2
   команду не доносит (см. выше), остаётся для не-Sunmi принтеров;
3. `rawbt:base64,…` с тем же импульсом;
4. пути нет (браузер без RawBT) — касса честно говорит об этом и в
   настройках, и тостом.

Подтверждение открытия. Мост v4 после команды опрашивает **датчик ящика**
(`getDrawerStatus`, тоже рефлексией) и отвечает:

| Ответ моста | Что это значит | Что видит кассир |
|---|---|---|
| `opened` | датчик подтвердил открытие | «Ящик открыт» |
| `drawer-did-not-open` | датчик говорит «закрыт» — команда ушла впустую | «Ящик не открылся» |
| `sent-unverified` | прошивка датчик не отдаёт | «Команда на открытие отправлена» |

Поэтому `awaitPrintResult` для ящика работает в режиме «молчание = команда
ушла» (таймаут 3 с), а слово «открыт» касса произносит только по датчику.
Окончательную проверку всё равно даёт ручной прогон на T2.

Когда ящик открывается сам (настройка «Открывать при наличных»): оплата
наличными (в момент подтверждения оплаты, не дожидаясь сети), возврат
наличными, внесение и изъятие, открытие смены и вход в пересчёт при
закрытии. Ручное открытие без продажи — кнопка на экране продажи и на
экране смены, право `cash_movement`.

Каждое открытие пишется в `drawer_opens` (см. `docs/database.md`);
без сети запись уходит в офлайн-очередь операцией `drawer.open`.

## Кухонный тикет и перепечатка

Тикет печатается автоматически (тумблер устройства «Тикет на кухню»): при
оплате, при дозаказе стола (только новые позиции) и при принятии
онлайн-заказа. Всегда на иврите, без цен.

Потерянный тикет перепечатывается из снапшота заказа в БД
(`kitchenTicket.ts`), включая заметки позиций:

- История операций → детали заказа → «Тикет на кухню» — весь заказ;
- зал → длинный тап по занятому столу → «Тикет на кухню» — весь текущий
  открытый счёт одним тикетом.

Перепечатка — чисто локальная печать: ничего не пишет в БД, не появляется на
экране бариста и не имеет пометки «повтор» — тикет выглядит как оригинал.
Работает только по тихим путям; в браузерном режиме показывается тост.
...
- стартовый экран;
- ориентация;
- ширина ленты 58/80 мм — применяется ко ВСЕМ печатным документам: рендеры
  `printCanvas.ts` читают её из `deviceStore` (80 мм → 576 px, 58 мм → 384 px
  @203dpi; на 58 мм кегли ужаты на 15%, колонки таблицы сужены). Тестовая
  печать выводит строку `רוחב נייר` для сверки настройки с фактической бумагой;
- режим печати;
- автопечать, вопрос о чеке, кухонный тикет;
- денежный ящик: подключён/нет, автооткрытие при наличных, контакт разъёма;

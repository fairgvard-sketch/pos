...
- `require_backoffice_or_staff(p_session, p_perm)` (091) — единый гейт ЗАПИСИ:
  сперва пускает владельца/менеджера бэкофиса без PIN, иначе — прежний
  `require_staff_perm`. Возвращает «автора» операции: `organization_members.staff_id`

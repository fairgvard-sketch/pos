...
⚠️ `require_staff_perm` СЕЙЧАС В МЯГКОМ РЕЖИМЕ, включая production. Миграция
045 заменила NULL-ветку на `RAISE`, но 055 переопределила функцию телом 044
(правился только фолбэк-CASE для `stock_take`) и вернула `RETURN NULL`.
Следствие: вызов привилегированного RPC БЕЗ токена сессии проходит — гейт
держит только переданный битый токен. Это не решение, а незамеченный регресс
055; восстановление строгого режима — отдельная миграция с предусловиями 045
(все кассы на новом клиенте, офлайн-очереди пусты). До неё нельзя считать
серверную проверку прав действующей и опираться на неё в новых фичах.

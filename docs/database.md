...
### Device identity (169, выпуск сверять с журналом)

Для authenticated JWT с точкой `auth_org_id()` проверяет живой Auth-аккаунт,
отсутствие действующего бана и соответствие org/location. При отказе оба
helper возвращают NULL: старый подписанный токен удалённой выделенной кассы
больше не читает организацию и не восстанавливает строку через регистрацию.
Архив остаётся косметическим; удаление shared/human account не обещает
индивидуального отзыва. Web-членство и PIN остаются отдельной моделью.

`register_device` обновляет конфликтующую `(org_id,device_uuid)` только при
том же `auth_user_id` и `location_id`; иначе `device_identity_conflict`.
Триггер `device_tenant_reference` проверяет принадлежность location к org
при INSERT/изменении этих полей. Исторические строки не переписываются.
Перед применением проверить существующие привязки и свежий backup; конфликты
не лечить удалением финансовых записей, Auth-пользователей или outbox.
Матрица/совместимость — [account-access](../../anglesite/docs/account-access.md).
...
Организация — граница арендатора; RLS использует `auth_org_id()`, а RPC и
поточечные product-гейты дополнительно проверяют location. Точка — филиал
внутри организации, и филиалы одной сети видят общие данные намеренно
...
- `register_device` идемпотентен по `(org_id, device_uuid)` только для того же
  Auth-user и точки (169), а UUID не является разрешением перехвата;
...
- `save_role`, `delete_role`, `role_allows` (094) — кастомные роли: именованный
  ...
  сервером.
  ⚠️ `roles.base` НИЧЕГО не решает: комментарий в 094 обещает фолбэк «если
  право не описано явно», но его нет — при `role_id IS NOT NULL` ответ даёт
  только `roles.perms`, иначе `forbidden`. Колонку пишет `save_role` (аргумент
  обязателен, проверка `manager|barista`), не читает никто: ни
  `require_staff_perm`, ни `verify_staff_pin`, ни касса. Поле убрано из формы
  роли в бэкофисе; менять сам столбец незачем — миграции forward-only;
...
- `delete_device_web(device_id, staff_session)` (135) — окончательное
  ...
  `supabase/tests/device_delete.test.sql`; старый JWT после удаления/бана
  отдельно проверен в `device_account_boundaries.test.sql` (169).
...
### Слепок эмитента в документе (150/151)

- `orders.issuer_name / issuer_tax_id / issuer_address` и те же три колонки
  на `refunds` — кто выпустил документ, **на момент выпуска**. До 150 личность
  эмитента жила только колонками `locations`, и печать с выгрузкой читали её
  живой: смена реквизитов задним числом меняла содержимое уже выпущенных
  חשבונית и уже сданных наборов מבנה אחיד.
- Снимается триггером в момент, когда заказ получает `receipt_number`
  (`trg_orders_issuer_insert` / `trg_orders_issuer_number`), и внутри
  `refunds_assign_document_number` — там же, где возврату выдаётся номер.
  Открытый заказ и отменённый до оплаты слепка не получают: документом они не
  стали. У временного офлайн-чека слепка тоже нет — он появится при
  синхронизации.
- Хранится **разрешённое** значение, а не сырая колонка: на бумагу уходит
  `COALESCE(receipt_business_name, name)`, и в слепке лежит то же самое, иначе
  перепечатка разойдётся с оригиналом.
- Три поля, а не пять: имя, ח.פ и адрес — то, что делает документ документом,
  и ровно то, что берёт выгрузка УФ. Телефон и подпись внизу чека —
  оформление (то же разделение, что в кабинете на Legal details и Receipt
  appearance), они читаются живыми.
- Читающая сторона: `printCanvas.ts` печатает шапку чека и зикуя из слепка с
  фолбэком на точку (Z-отчёт не трогали — это не фискальный документ);
  `uf_export_documents_for` отдаёт эмитента каждого документа (151).
- `uf_export_issuers_for/_web/(session)` — сколько РАЗНЫХ эмитентов в периоде.
  Заголовок מבנה אחיד несёт один ח.פ, поэтому период, внутри которого
  реквизиты менялись, Edge Function отдавать отказывается
  (`issuer_changed_in_period`, 422): усреднить двух эмитентов нельзя, период
  выгружается раздельно.
- Backfill 150 проставил уже выпущенным документам СЕГОДНЯШНИЕ реквизиты
  точки. Это не делает их правильными задним числом — подлинных значений
  никто не сохранял, в том и был дефект; backfill лишь останавливает
  дальнейший дрейф.

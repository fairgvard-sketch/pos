...
Ящик подключается к порту принтера, поэтому открывается тем же мостом:

- `KassaAndroid.openDrawer(jobId)` (мост v4, APK 1.7+) — сначала штатный
  `openDrawer` сервиса печати Sunmi (берётся рефлексией: метода нет в части
  прошивок), затем фолбэк импульсом `ESC p 0 25 250`;
- на **старом APK ничего не ломается и обновление не обязательно**: если
  метода нет, веб-часть шлёт тот же импульс через `printBase64` — в задании
  нет ни растра, ни отреза, поэтому бумага не двигается;
- нестандартный контакт разъёма (5 вместо 2) задаётся в кассе
  (Настройки → Устройство → «Денежный ящик») и всегда идёт импульсом.

Подтвердить ФИЗИЧЕСКОЕ открытие ящика этот путь не может: колбэк говорит
лишь о том, что команда принята принтером. Открытие проверяется руками —
см. `docs/t2-smoke-test.md`.

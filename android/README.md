...
- `KassaAndroid.openDrawer(jobId, pin)` (мост v4, APK 1.7+) — штатный
  `openDrawer` сервиса печати Sunmi (берётся рефлексией: метода нет в части
  прошивок), фолбэк — импульс `ESC p m 25 250` напрямую через `sendRAWData`,
  минуя transaction-буфер;
- итог проверяется датчиком `getDrawerStatus`: `opened` (подтверждено),
  `drawer-did-not-open` (команда ушла впустую) или `sent-unverified`
  (прошивка датчик не отдаёт);
- **на APK < 1.7 ящик на Sunmi не работает**: импульс через обычное задание
  печати сервис принимает, но до порта не доносит — проверено на T2 mini
  06.08.2026. Путь через `printBase64` остаётся для не-Sunmi принтеров.

Открытие проверяется руками — см. `docs/t2-smoke-test.md`.

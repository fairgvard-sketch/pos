...
export interface PublicMenu {
  location: {
    ...
    currency: string
    /** Открыто ПРЯМО СЕЙЧАС: часы работы (+ открытая смена для POS-точки) */
    is_open: boolean
    /**
     * Предзаказ (116): точка закрыта сейчас, но заявку «ко времени» внутри
     * будущего окна сервер примет — оформление остаётся доступным. Отсутствие
     * поля (старая edge function) = ведём себя как раньше, по is_open.
     */
    preorder?: boolean
    /**
    ...
    /**
     * Часы работы (112): ключ — день недели (0 = воскресенье) в таймзоне
     * точки, значение — окна [["08:00","20:00"], ...]. Конец меньше начала
     * = переход через полночь. null/отсутствует — приём в любое время.
     */

-- ============================================================
-- 162: «обычно» только там, где привычка действительно есть
--
-- ЗАЧЕМ.
--
-- На живой приёмке карточка утверждала «usually Tue 08:00» у гостя,
-- чьи редкие визиты приходились на разные дни и часы. Утверждение было
-- выдумано механикой 156:
--
--   * день и час выбирались ДВУМЯ независимыми запросами;
--   * ничья решалась произвольно — побеждал меньший номер;
--   * независимо выбранные день и час складывались в пару, которой в
--     визитах не было никогда;
--   * одного визита хватало, чтобы объявить привычку.
--
-- Подсказка, которой нельзя верить, хуже отсутствующей: хостес по ней
-- готовит стол.
--
-- ПРАВИЛО.
--
-- Пара «день + час» берётся ЦЕЛИКОМ, в часах точки, и возвращается
-- только когда победившая корзина встречалась не меньше двух раз и
-- строго чаще следующей. Иначе день и час пусты, и интерфейс просто не
-- показывает чип.
--
-- Средний размер компании считается отдельно и не подавляется: «обычно
-- вчетвером» полезно и без привычки по времени.
--
-- ⚠️ ТРЕБУЕТ 156 и 161. Обе НЕ редактируются — функция заменяется
-- целиком.
-- ============================================================
...
BEGIN
  ...
  /*
   * Привычка гостя — совместная корзина «день + час» в часах ТОЧКИ.
   *
   * ЧТО БЫЛО СЛОМАНО (156). День и час выбирались ДВУМЯ независимыми
   * запросами, каждый — по своему максимуму с добором по меньшему
   * номеру. Отсюда три беды сразу:
   *
   *   * ничья решалась произвольно — побеждал меньший номер дня;
   *   * независимо выбранные день и час складывались в пару, которой
   *     в визитах не было НИКОГДА;
   *   * один-единственный визит объявлялся привычкой.
   *
   * На живой приёмке это дало «usually Tue 08:00» у гостя, чьи редкие
   * визиты приходились на разные дни и часы.
   *
   * ТЕПЕРЬ пара берётся целиком и только при внятном большинстве:
   * победитель встречался не меньше двух раз И строго чаще следующей
   * корзины. Ничья привычкой не считается — «обычно» без основания
   * хуже, чем молчание.
   */
  SELECT b.dow, b.hour INTO v_slot
  FROM (
    SELECT dow, hour, n,
           ROW_NUMBER() OVER (ORDER BY n DESC, dow, hour) AS rn,
           LEAD(n)      OVER (ORDER BY n DESC, dow, hour) AS runner_up
    FROM (
      SELECT EXTRACT(DOW FROM r.reserved_at AT TIME ZONE
                       COALESCE(NULLIF(l.timezone, ''), 'Asia/Jerusalem'))::INTEGER AS dow,
             EXTRACT(HOUR FROM r.reserved_at AT TIME ZONE
  ...
  WHERE b.rn = 1 AND b.n >= 2 AND (b.runner_up IS NULL OR b.n > b.runner_up);

  /*
   * Средний размер компании считается ОТДЕЛЬНО и не зависит от того,
   * сложилась ли привычка по времени: «обычно вчетвером» — полезная
   * подсказка и у гостя, который ходит в разные дни.
   */
  SELECT jsonb_build_object(
           'dow',   v_slot.dow,
           'hour',  v_slot.hour,
...
COMMENT ON FUNCTION get_guest_card(UUID, INTEGER) IS
  'Карточка клиента (114/121/156/161/162). usual.dow/hour — совместная корзина дня и часа в зоне точки, только при большинстве (>=2 и строго больше следующей); иначе NULL.';

-- ============================================================
-- ОТКАТ
--
-- Forward-only, схема не менялась. Откат — вернуть тело 161 новой
-- миграцией; тогда вернётся и выдуманная привычка.
--
-- ПРОВЕРКА под веб-владельцем:
--   SELECT get_guest_card('<guest>') -> 'usual';
-- у гостя с одним визитом ожидается dow/hour = null, party — число.
-- ============================================================

-- ============================================================
-- 059 RESERVATION HOURS — часы приёма броней (enforcement).
--
-- Дополняет публичную бронь (053): если владелец задал часы приёма
-- (locations.settings.reservations.open/close, 'HH:MM' локального
-- времени точки), submit_reservation отклоняет брони с временем вне
-- окна — код 'outside_hours'. Обе границы не заданы = ограничения нет
-- (поведение как раньше). Гостевая страница дублирует ограничение,
-- предлагая слоты только внутри часов; сервер — последняя проверка.
--
-- Пересоздаём submit_reservation с той же 7-арг сигнатурой (CREATE OR
-- REPLACE, без новой перегрузки) — тело копия 053 + проверка часов.
-- ============================================================

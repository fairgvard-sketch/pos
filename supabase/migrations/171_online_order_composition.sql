-- 171: состав гостевой заявки проверяется сервером (B5).
--
-- 116 считает цены сервером и проверяет принадлежность модификатора товару,
-- его доступность и предел 10 различных модификаторов на позицию. Чего не
-- было: min_select/max_select групп и обязательности размера. SQL-проба на
-- схеме 170 (синтетическая БД) принимала все четыре негодных состава:
--   * позиция без обязательной группы (min_select = 1) — принята;
--   * два модификатора в группе с max_select = 1 — принята;
--   * три модификатора в группе с max_select = 2 — принята;
--   * товар, у которого есть только размеры, без variant_id — принят по
--     базовой цене товара (800 вместо 1400 за единственный размер).
--
-- Клиентская карточка эти правила соблюдает, но клиент границей не является:
-- заявку шлёт кто угодно, включая старый бандл и прямой вызов PostgREST.
--
-- Подпись, формат ответа и остальные инварианты не меняются: org/tenant,
-- capability, смена и часы работы, table-token, лимит частоты заявок,
-- идемпотентность по client_uuid, DISTINCT-предел 10 модификаторов и
-- серверный расчёт цены остаются как в 116.
--
-- Новый код ошибки: invalid_composition: <группа или товар>. Гость видит
-- «состав изменился — выберите заново», а не общий сбой.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    ELSE
      -- Размер обязателен, если он есть у товара: иначе заявка уходит по
      -- базовой цене, а гость получает позицию, которой в каталоге в таком
      -- виде нет (в меню остались только размеры).
      IF EXISTS (SELECT 1 FROM item_variants WHERE item_id = v_mi.id) THEN
        RAISE EXCEPTION 'invalid_composition: %', v_mi.name;
      END IF;
      v_unit := v_mi.price;
    END IF;
    ...
    -- Минимум и максимум каждой группы товара. Карточка гостя их соблюдает,
    -- но заявка приходит из недоверенного источника: старый бандл, открытая
    -- карточка со снятым составом, прямой вызов PostgREST. Проверяются ВСЕ
    -- группы товара, а не только присланные: пустой modifier_ids тоже
    -- нарушает обязательную группу.
    FOR v_group IN
      SELECT g.id, g.name, g.min_select, g.max_select
      FROM menu_item_modifier_groups mimg
      JOIN modifier_groups g ON g.id = mimg.group_id
      WHERE mimg.item_id = v_mi.id
        AND mimg.org_id = v_loc.org_id
    LOOP
      SELECT COUNT(*) INTO v_picked
      FROM modifiers m
      WHERE m.group_id = v_group.id
        AND m.id = ANY(v_mod_ids);
      -- max_select = 0 — «без верхнего ограничения»: тот же контракт, что у
      -- карточки гостя и у сверки корзины.
      IF v_picked < v_group.min_select
         OR (v_group.max_select > 0 AND v_picked > v_group.max_select) THEN
        RAISE EXCEPTION 'invalid_composition: %', v_group.name;
      END IF;
    END LOOP;

-- ============================================================
-- 012 TABLE LABEL — номер/подпись стола на заказе (режим counter_tables).
--
-- Уровень 1 «столов»: точка в режиме counter_tables указывает, КУДА
-- нести заказ (номер стола / «терраса»). Оплата по-прежнему сразу,
-- заказ так же уходит в очередь бариста — модель не меняется, лишь
-- добавляется снапшот-подпись.
--
-- Полный режим tables (карта зала, FK table_id, открытые счета) —
-- отдельная миграция позже. Здесь намеренно только текстовая подпись,
-- чтобы не заводить пустую таблицу столов раньше времени.
-- ============================================================

ALTER TABLE orders ADD COLUMN table_label TEXT;

-- ============================================================
-- place_order — добавляем p_table_label. Новая сигнатура → старую
-- 6-аргументную версию (из 011) удаляем явно (перегрузка по сигнатуре).
-- ============================================================
DROP FUNCTION IF EXISTS place_order(UUID, UUID, TEXT, TEXT, JSONB, JSONB);

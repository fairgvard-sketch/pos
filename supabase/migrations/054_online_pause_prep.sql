-- ============================================================
-- 054 — Пауза онлайн-заказов + время приготовления (идея из Square).
--
-- Экран /online: статус «Приём включён» кликабелен — пауза на
-- 30 мин / 1 ч / 2 ч / до конца дня, снимается сама (сравнение
-- по времени, без крона). Отдельно — «время приготовления», его
-- видит гость на странице заказа.
--
-- Хранение — locations.settings.online_orders (паттерн 051):
--   { "paused_until": "2026-07-12T14:30:00+00:00", "prep_minutes": 20 }
-- Отсутствие ключей = пауза не стоит, время приготовления не задано.
--
-- Пишут точечные RPC (не update_location_config: тот требует право
-- 'manage', а паузу должен уметь ставить и бариста в запаре). Право —
-- 'online_pause', дефолт require_staff_perm — 'all', ограничивается
-- в Настройки → Права доступа.
--
-- Enforcement: submit_online_order отклоняет заявку кодом 'paused'
-- (тело — копия 051 + одна проверка); public-menu отдаёт accepting
-- с учётом паузы + paused_until/prep_minutes (деплой edge function!).
-- ============================================================

-- ── Пауза приёма: NULL = возобновить ────────────────────────
CREATE OR REPLACE FUNCTION set_online_pause(
...
BEGIN
  ...
  -- Пауза только в будущее и не дольше суток с хвостом («до конца дня»
  -- из другого пояса укладывается в 36 часов с запасом)
  IF p_paused_until IS NOT NULL
     AND (p_paused_until <= NOW() OR p_paused_until > NOW() + INTERVAL '36 hours') THEN
  ...
  UPDATE locations SET settings = jsonb_set(
    ...
    '{online_orders}',
    COALESCE(settings -> 'online_orders', '{}'::jsonb)
      || CASE WHEN p_paused_until IS NULL
           THEN '{"paused_until": null}'::jsonb
           ELSE jsonb_build_object('paused_until', p_paused_until)
         END
  )
  WHERE id = v_loc;
END $$;

REVOKE EXECUTE ON FUNCTION set_online_pause FROM anon, public;

-- ── Время приготовления, минуты (гость видит при заказе) ────
...
BEGIN
  ...
  IF p_minutes IS NULL OR p_minutes < 0 OR p_minutes > 180 THEN
  ...
  UPDATE locations SET settings = jsonb_set(
    ...
    '{online_orders}',
    COALESCE(settings -> 'online_orders', '{}'::jsonb)
      || jsonb_build_object('prep_minutes', p_minutes)
  )
  WHERE id = v_loc;
END $$;
...
-- ── submit_online_order: + отказ 'paused' (тело — копия 051) ─
...
BEGIN
  ...
  -- Пауза (054): приём приостановлен с кассы, вернётся сам
  IF COALESCE((v_loc.settings -> 'online_orders' ->> 'paused_until')::TIMESTAMPTZ > NOW(), FALSE) THEN

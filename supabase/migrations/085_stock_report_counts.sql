-- 085: stock_report отдаёт counts — число инвентаризационных строк периода.
--
-- Отчёт «теория vs факт» (P2): расхождение позиции — это count_adj, но нулевая
-- сумма поправок неотличима от «не проверяли вовсе» (инвентаризация без
-- расхождения пишет count-строку с qty_delta = 0). counts отличает «сошлось»
-- от «нет данных» — вторые не должны выглядеть идеальными.
-- Тело функции — из 078, добавлено только поле counts.
  ...
  agg AS (
    SELECT m.menu_item_id,
           ...
           -COALESCE(SUM(m.qty_delta) FILTER (WHERE in_period AND m.type = 'waste'), 0)           AS waste,
            ...
            COUNT(*) FILTER (WHERE in_period AND m.type = 'count')                                AS counts,
